...
struct alloc_s {
	...
	int (*realloc)(alloc_t *alloc, void **ptr, size_t *old_size, size_t new_size);
...
#define alloc_realloc(_alloc, _ptr, _old_size, _new_size) ((_alloc)->realloc ? (_alloc)->realloc(_alloc, _ptr, _old_size, _new_size) : 1)
#define alloc_free(_alloc, _ptr, _size)                                                                                                    \
	if ((_alloc)->free) {                                                                                                              \
		(_alloc)->free(_alloc, _ptr, _size);                                                                                       \
	}
